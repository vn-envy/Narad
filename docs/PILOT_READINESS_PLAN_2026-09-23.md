...
> **Status (2026-09-24): built; closes Fix G.** Details in `docs/WORKFLOW_PATHS.md`.
> - **Evidence, not text.** Every stage of the six packs (version 2) declares `done_when`. `workflow_evidence.py` checks it only against what the Mac can verify in the run owner's own stores: tool receipts recorded from real function responses in a turn bound to the run, saved reviews with their `health.db`/`finance.db` rows, `kriya.db` tasks, Anumati proposals, files under `artifacts/runs/<profile>/`, the person's own tap, the Gurukul loop. The avatars report with `report_stage_result` (`done | needs_input | blocked | in_progress`); a reply's text never completes a stage, and the "any non-empty reply completes the stage" path is gone. Evidence that lands later (a review saved on the phone, a task that finishes) settles on the next read, Kala tick, or review save.
> - **Intent → path.** A table of English, Hinglish and Hindi phrases (`phase-1/path_intent.py`) behind `prerouter.suggest_path`; unbound turns get a `path_suggestion` card (Start / Continue here / Not now), once per path per thread, and never auto-start. Binding is per thread on the server.
> - **Mobile-first intake.** A chat start opens on intake, prefilled from the phone's time zone and the last run of the path, and asks one or two questions at a time; the Paths screen does the same. No intake field is a Mac path; files come as chat attachments.
> - **Per-path tooling.** Health: lab report → `extract_fields` → confirmed save (`records_saved(lab_results)`) → `get_lab_results` trend, and per-profile medicine reminders (Kala fires them for their own profile). Finance: statement CSV (`import_csv`) or photo/PDF (`extract_fields`, now also on Rama) → confirmed ledger rows → budget and goal tools. Travel: live search through a Kriya task, a price watch that only appends findings, a trip pack artifact. Career: an application tracker (`track_application`) and a read-only Gmail reply check. Documents: numbered versions and an export zip with `provenance.md`. Teach: the guided loop's graded answers are the evidence.
> - **Paths screen.** Each stage shows its finish line in plain words; the current one shows which conditions hold, its evidence (review, task, approval, file links), and what's next with the person's own controls (answer, confirm the values, watch the task, approve, export, "I did this" where accepted, skip where optional).
> - **Gate evidence** (`phase-9/test_workflow_paths.py`, `test_workflow_engine.py`, `test_workflow_api.py`, `phase-1/test_workflow_chat_turns.py`, `test_path_intent.py`): all six paths complete end to end on stubbed tools with evidence; 0 false completions across free text, a bare "done", a failed or unconfigured tool, another profile's review/task/file, an unconfirmed review, a task from before the run or still running, a file outside the artifact folder, and a free "complete" from the API (409); check-ins and watches never rewind a run or clear an approval.
> - **Still open:** fares are recorded as the task's answer text, not normalised records; the Gmail reply check matches company names, not addresses; saving a relative's report into their own profile (a care-circle decision); the live smoke script (`scripts/run_workflow_paths_e2e.py`) now checks refusals rather than walking stages.
