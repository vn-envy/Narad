...
from smriti_indexer import fts_reindex_wiki_page, schedule_index_refresh
...
def _append_to_wiki(user_id: str, entity_type: str, entry: str, project_id: str = "general") -> None:
    ...
    with open(path, "a") as f:
        f.write(f"\n{header}{entry.strip()}\n")
    _index_wiki_page(user_id, project_id, path)
...
def _index_wiki_page(user_id: str, project_id: str, path: Path) -> None:
    """Synchronous lexical indexing — pure SQLite, no network, must never
    break a wiki write."""
    try:
        fts_reindex_wiki_page(user_id, project_id, path)
    except Exception as exc:
        import logging

        logging.getLogger("narad.smriti").warning(
            "Smriti: wiki FTS index failed for %s: %s", path.name, exc
        )
