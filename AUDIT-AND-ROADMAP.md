...
- **M2 — Memory consolidation (1 week).** ✅ *Code done 2026-07-05 (commits M2.1–M2.3).* §5 delivered: **M2.1** — content-hash incremental indexing (byte-offset checkpoint, O(1) no-op recall; wiki sections re-embed only on change) + one pinned embedding model per process (`smriti_embed.py`; no silent hash fallback — keyless pins `local` loudly, provider failure raises visibly). **M2.2** — 3-plane merge: `capture_episode` is the only write (episodes.jsonl + episode FTS5 + vector manifests); one ranked recall merges vector+lexical with relevance floor (`NARAD_RECALL_FLOOR`=0.30) and "recalled from <date>" provenance; `forget()` cascades across all planes; karma writes one ledger (merge-read keeps history); scribe compiles wiki from real episode results; `/memory` + `/search` read the canonical store — LanceDB out of every hot path. **M2.3** — supervisor recall: `[NARAD MEMORY]` packet (episodes+wiki+sutras+Sankalpa, `NARAD_SUPERVISOR_RECALL_BUDGET`=384) prepended each turn + `narad_recall` SSE. *Left for M3: wiki regeneration on forget (Swapna owns derived pages); legacy `phase-2/smriti.py` module removal once nothing imports it.*
- **M3 — Close the loops (1 week).** ✅ *Code done 2026-07-05/06 (commits M3.1–M3.4).* §6.3 delivered: **M3.1** — `vahana.py` delivery channel: inbox store + env-gated ntfy push, `/inbox` endpoints in server. **M3.2** — `kala_scheduler.py`: reminders actually fire, Swapna digest consumed nightly; medication reminders read from health.db and deliver via Vahana (health/finance DB paths now respect NARAD_HOME). **M3.3** — Andon feeds the retry path; `TOOL_ERROR` surfaces only when unrecovered. **M3.4** — `mail_triage_skill.py`: read-only IMAP triage for Krishna (BODY.PEEK, never marks seen; rule-based urgent/action/finance/calendar/newsletter/social/other; env-gated, reuses email_skill creds; `deliver=True` pushes digest via Vahana). *Left for M4+: wiki regeneration on forget. Legacy `phase-2/smriti.py` removed 2026-07-09 (was unimported).*
- **M4 — Prove the moat (ongoing).** ◐ Cost ledger ✅ (M4.1, incl. avatar spans), golden-task CI ✅ (M4.2, 48 tasks), sutra A/B ✅ (M4.3). *Open: M4.4 distilled-rule sutras.*
- **M5 — Pratham + Sopan (packaging & tiered access).** Tauri packaging + tiered brains so anyone can run Narad by spend/hardware: T0 edge (Gemma 4 E2B/E4B), T1 local default (**Gemma 4 12B QAT**, Apache 2.0), T2 BYO key, T3 Claude-subscription via Agent SDK credits, T4 hybrid — plus budget guardrails. *Detailed plan: GURU-AND-ONBOARDING-PLAN.md (Pratham O1–O7 + Sopan S1–S4).*
- **M6 — Guru (teach panel v2).** ◐ G1–G4 ✅ shipped 2026-07-08/09 (guru_engine syllabus DAG + ELI5 ladder, LLM artifacts, learner_state + grading, Gurukul tab). *Open: G6 complete teach skill in chat (frontier-atom packet, teaching persona, in-chat mastery loop), G5 spaced-repetition wiring via Kala→Vahana, G7 guru golden tasks — see GURU-AND-ONBOARDING-PLAN.md Part D.*
