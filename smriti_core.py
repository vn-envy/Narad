...
def capture_episode(
    *,
    ...
    remembered = False
    try:
        from smriti import remember
        remember(task, result, avatar, user_id=user_id)
        remembered = True
        log_mutation(
            "semantic_memory_written",
            entity_type="smriti_memory",
            entity_id=episode.id,
            actor=avatar,
            detail=task,
            provenance_ids=episode.provenance,
...
async def recall_context(
    ...
    *,
    ...
    try:
        ...
        from smriti import recall, recall_exact

        memories = recall(query, user_id=user_id)
        if memories:
            block = _fit_block(
                "smriti_semantic_legacy",
                "[MEMORY — what this user has done before on related topics]\n"
                f"{memories}\n[END MEMORY]"
            )
            if block:
                blocks.append(block)
                provenance.append({"kind": "smriti_semantic_legacy", "preview": _truncate(block, 200)})

        if avatar == "Parashurama":
            exact_hits = recall_exact(query, user_id=user_id, avatar=avatar)
            if exact_hits:
                block = _fit_block(
                    "smriti_exact",
                    "[EXACT-MATCH MEMORY — prior code snippets / error messages]\n"
                    f"{exact_hits}\n[END EXACT-MATCH MEMORY]"
                )
                if block:
                    blocks.append(block)
...
def forget(entity_id: str, *, entity_type: str = "generic", actor: str = "Narad") -> dict[str, Any]:
    tombstone = {
        "id": str(uuid.uuid4()),
        "ts": datetime.now(timezone.utc).isoformat(),
        "action": "forgotten",
    ...
    return {"status": "ok", "tombstone_id": tombstone["id"]}
