                      ...
                      return (
                        <div key={stage.id} style={{ display: 'grid', gridTemplateColumns: '24px minmax(0,1fr)', gap: 9, minHeight: 48 }}>
                          ...
                          <div style={{ paddingBottom: 12, opacity: !done && !current ? 0.52 : 1 }}>
                            <div style={{ display: 'flex', gap: 7, alignItems: 'baseline', flexWrap: 'wrap' }}><span style={{ fontSize: 11.5, fontWeight: current ? 800 : 650 }}>{stage.title}</span><span style={{ fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.43)' }}>{stage.owner}</span></div>
