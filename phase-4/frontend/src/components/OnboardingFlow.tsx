...
function StatusRow({ ready, title, detail, optional = false }: { ready: boolean; title: string; detail: string; optional?: boolean }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) auto', gap: 10, alignItems: 'center', padding: '11px 0', borderBottom: '1px solid rgba(var(--rgb-ink),0.08)' }}>
      ...
      <span>
        ...
        <span style={{ display: 'block', marginTop: 2, fontSize: 10.5, lineHeight: 1.35, color: 'rgba(var(--rgb-ink),0.5)' }}>{detail}</span>
      </span>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: ready ? 'var(--tulsi)' : 'rgba(var(--rgb-ink),0.38)' }}>
        ...
        <section style={{ minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          <header style={{ flexShrink: 0, padding: isMobile ? '16px 17px 12px' : '20px 25px 14px', borderBottom: '1px solid rgba(var(--rgb-ink),0.08)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 5, marginTop: 13 }}>
              {[0, 1, 2, 3].map(index => <span key={index} style={{ height: 3, borderRadius: 99, background: index <= step ? 'var(--sindoor)' : 'rgba(var(--rgb-ink),0.09)', transition: 'background 180ms ease' }} />)}
            </div>
          </header>
          ...
          <div className="panel-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: isMobile ? '24px 18px 28px' : '31px 34px 34px' }}>
            {step === 0 && (
              <div style={{ maxWidth: 500 }}>
                ...
                <p style={{ margin: '11px 0 0', maxWidth: 470, fontSize: 12.5, lineHeight: 1.65, color: 'rgba(var(--rgb-ink),0.58)' }}>Narad coordinates research, planning, creation, and computer work. Tell it what to call you; folders and documents can be attached directly when they matter.</p>
                <label style={{ display: 'grid', gap: 7, marginTop: 26 }}>
                  <span style={{ fontSize: 10.5, fontWeight: 750, color: 'rgba(var(--rgb-ink),0.62)' }}>What should Narad call you? <span style={{ fontWeight: 500, color: 'rgba(var(--rgb-ink),0.38)' }}>(optional)</span></span>
                  <input autoFocus value={displayName} onChange={event => setDisplayName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void continueFromWelcome() }} placeholder="Your name" maxLength={80} style={{ width: '100%', padding: '12px 13px', borderRadius: 10, border: '1px solid rgba(var(--rgb-ink),0.15)', background: 'rgba(var(--rgb-surface),0.58)', color: 'var(--kajal)', fontSize: 13, outline: 'none' }} />
                </label>
                <div style={{ marginTop: 16, padding: '11px 12px', display: 'flex', gap: 9, borderRadius: 10, background: 'rgba(53,94,59,0.065)', color: 'rgba(var(--rgb-ink),0.58)', fontSize: 10.5, lineHeight: 1.45 }}><LockKeyhole size={14} style={{ flex: '0 0 auto', color: 'var(--tulsi)', marginTop: 1 }} /> Narad stores setup, memory, and uploaded files locally. Only requests to services you connect leave this machine.</div>
            ...
            {step === 1 && (
              <div style={{ maxWidth: 520 }}>
                ...
                <p style={{ margin: '10px 0 0', fontSize: 12.5, lineHeight: 1.6, color: 'rgba(var(--rgb-ink),0.56)' }}>{localModel?.ready ? 'Gemma 4 runs on this machine with no key, subscription, or data leaving Narad.' : brainReady ? `Narad can use ${modelConnections.map(providerLabel).join(', ')}. Add the offline model below whenever you want a local fallback.` : 'Download Narad\'s local Gemma 4 brain once. Cloud connections remain optional accelerators.'}</p>

                <div style={{ marginTop: 20, padding: '13px 14px', borderRadius: 13, border: `1px solid ${localModel?.ready ? 'rgba(53,94,59,0.25)' : 'rgba(var(--rgb-ink),0.13)'}`, background: localModel?.ready ? 'rgba(53,94,59,0.065)' : 'linear-gradient(135deg, rgba(var(--rgb-ink),0.04), rgba(194,65,12,0.045))' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '36px minmax(0,1fr) auto', gap: 11, alignItems: 'center' }}>
                    ...
                    <span><span style={{ display: 'block', fontSize: 12, fontWeight: 780, color: 'var(--kajal)' }}>Gemma 4 {localModel?.model_size || 'E2B'} · Q4</span><span style={{ display: 'block', marginTop: 2, fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.5)' }}>{localModel?.ready ? `Offline · ${Math.round((localModel.configured_context_tokens || 16384) / 1024)}K working context · tools + vision` : `${localModel?.download_gb || 7.2} GB package · selected for ${Math.round(localModel?.ram_gb || 8)} GB RAM`}</span></span>
                    {localModel?.ready ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'var(--tulsi)', textTransform: 'uppercase' }}>ready</span> : !isOwner ? <span style={{ fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.5)' }}>Owner adds it</span> : localModel?.runtime_installed ? <button type="button" onClick={() => void installOfflineModel()} disabled={busy !== null || localModel.install.state === 'running'} style={{ border: 0, borderRadius: 8, padding: '8px 11px', background: 'var(--tulsi)', color: '#fff', fontSize: 9.5, fontWeight: 750, cursor: busy || localModel.install.state === 'running' ? 'wait' : 'pointer' }}>{busy === 'local' || localModel.install.state === 'running' ? `${Math.round((localModel.install.progress || 0) * 100)}%` : 'Download'}</button> : <a href="https://ollama.com/download" target="_blank" rel="noreferrer" style={{ borderRadius: 8, padding: '8px 10px', background: 'var(--kajal)', color: 'var(--paper)', fontSize: 9.5, fontWeight: 750, textDecoration: 'none' }}>Get runtime</a>}
                  </div>
                  {localModel?.install.state === 'running' && <div style={{ marginTop: 10, height: 4, overflow: 'hidden', borderRadius: 99, background: 'rgba(var(--rgb-ink),0.09)' }}><div style={{ width: `${Math.max(2, (localModel.install.progress || 0) * 100)}%`, height: '100%', borderRadius: 99, background: 'var(--tulsi)', transition: 'width 250ms ease' }} /></div>}
                  <div style={{ marginTop: 8, fontSize: 9.5, lineHeight: 1.4, color: 'rgba(var(--rgb-ink),0.46)' }}>{localModel?.install.state === 'running' ? localModel.install.status : localModel?.memory_constrained ? 'E2B loads after your first local request, then releases after use to keep the browser responsive.' : 'Narad detected at least 16 GB RAM and automatically selected the stronger E4B model.'}</div>
                </div>

                {disabledGrok && (
                  <div style={{ marginTop: 12, padding: '11px 12px', display: 'flex', gap: 9, alignItems: 'center', borderRadius: 10, background: 'rgba(var(--rgb-ink),0.045)', color: 'rgba(var(--rgb-ink),0.58)', fontSize: 10.5, lineHeight: 1.45 }}>
                    <LockKeyhole size={14} style={{ flex: '0 0 auto', color: 'rgba(var(--rgb-ink),0.42)' }} />
                ...
                {!brainReady && isOwner && (
                  <div style={{ marginTop: 16, display: 'grid', gap: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9, fontFamily: 'var(--font-mono)', fontSize: 8.5, letterSpacing: '0.09em', textTransform: 'uppercase', color: 'rgba(var(--rgb-ink),0.36)' }}><span style={{ flex: 1, height: 1, background: 'rgba(var(--rgb-ink),0.09)' }} /> or connect a cloud boost <span style={{ flex: 1, height: 1, background: 'rgba(var(--rgb-ink),0.09)' }} /></div>
                    <button type="button" onClick={() => setShowKeyForm(value => !value)} style={{ display: 'grid', gridTemplateColumns: '36px minmax(0,1fr) auto', gap: 11, alignItems: 'center', padding: '12px 13px', borderRadius: 12, border: '1px solid rgba(var(--rgb-ink),0.12)', background: 'rgba(var(--rgb-surface),0.55)', color: 'var(--kajal)', textAlign: 'left', cursor: 'pointer' }}>
                      ...
                      <span><span style={{ display: 'block', fontSize: 12, fontWeight: 750 }}>Connect an API key</span><span style={{ display: 'block', marginTop: 2, fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.48)' }}>Gemini, DeepSeek, OpenAI, or Claude</span></span>
                      <ArrowRight size={14} style={{ transform: showKeyForm ? 'rotate(90deg)' : undefined }} />
                    </button>

                    {showKeyForm && (
                      <div style={{ display: 'grid', gap: 8, padding: 12, borderRadius: 12, border: '1px solid rgba(var(--rgb-ink),0.1)', background: 'rgba(var(--rgb-ink),0.025)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '140px minmax(0,1fr)', gap: 8 }}>
                          <select value={provider} onChange={event => setProvider(event.target.value)} aria-label="Model provider" style={{ padding: '9px 10px', borderRadius: 8, border: '1px solid rgba(var(--rgb-ink),0.14)', background: 'var(--paper)', color: 'var(--kajal)', fontSize: 11.5 }}>
                            {providerOptions.map(item => <option key={item.provider} value={item.provider}>{item.label}</option>)}
                          </select>
                          <input type="password" value={keyInput} onChange={event => setKeyInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void connectKey(provider, keyInput) }} aria-label="Provider API key" placeholder="Paste the API key" style={{ padding: '9px 10px', borderRadius: 8, border: '1px solid rgba(var(--rgb-ink),0.14)', background: 'var(--paper)', color: 'var(--kajal)', fontFamily: 'var(--font-mono)', fontSize: 11 }} />
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
                          ...
                          <button type="button" onClick={() => void connectKey(provider, keyInput)} disabled={!keyInput.trim() || busy !== null} style={{ border: 0, borderRadius: 8, padding: '8px 12px', background: keyInput.trim() ? 'var(--sindoor)' : 'rgba(var(--rgb-ink),0.12)', color: keyInput.trim() ? '#fff' : 'rgba(var(--rgb-ink),0.4)', fontSize: 10.5, fontWeight: 700, cursor: keyInput.trim() ? 'pointer' : 'default' }}>{busy?.startsWith('key:') ? 'Testing...' : 'Test and save'}</button>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div style={{ marginTop: 23, display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap' }}>
                  <button type="button" onClick={() => setStep(2)} disabled={busy !== null || !brainReady} style={{ border: 0, borderRadius: 10, background: brainReady ? 'var(--tulsi)' : 'rgba(var(--rgb-ink),0.14)', color: brainReady ? '#fff' : 'rgba(var(--rgb-ink),0.42)', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 7, fontSize: 11.5, fontWeight: 750, cursor: busy || !brainReady ? 'default' : 'pointer' }}>{brainReady ? <Check size={14} /> : <Download size={14} />} {brainReady ? 'Brain ready' : 'Choose a model above'}</button>
                  <button type="button" onClick={() => setStep(0)} style={{ border: 0, background: 'transparent', color: 'rgba(var(--rgb-ink),0.48)', fontSize: 10.5, cursor: 'pointer' }}>Back</button>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div style={{ maxWidth: 550 }}>
                ...
                <p style={{ margin: '10px 0 0', fontSize: 12.5, lineHeight: 1.6, color: 'rgba(var(--rgb-ink),0.56)' }}>These permissions belong only to this family profile. Narad previews consequential actions and Jev evaluates computer and phone runs.</p>

                <div style={{ display: 'grid', gap: 10, marginTop: 18 }}>
                  <div style={{ padding: '13px 14px', borderRadius: 13, border: `1px solid ${googleWorkspace?.connected ? 'rgba(53,94,59,0.25)' : 'rgba(var(--rgb-ink),0.12)'}`, background: 'rgba(var(--rgb-surface),0.5)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '36px minmax(0,1fr) auto', gap: 11, alignItems: 'center' }}>
                      <span style={{ width: 35, height: 35, display: 'grid', placeItems: 'center', borderRadius: 10, background: 'rgba(194,65,12,0.08)', color: 'var(--sindoor)' }}><KeyRound size={17} /></span>
                      <span><span style={{ display: 'block', fontSize: 12, fontWeight: 760, color: 'var(--kajal)' }}>Google Workspace</span><span style={{ display: 'block', marginTop: 2, fontSize: 9.5, lineHeight: 1.35, color: 'rgba(var(--rgb-ink),0.5)' }}>Gmail, Calendar, Drive, and Photos with read-only consent first.</span></span>
                      {googleWorkspace?.connected ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'var(--tulsi)', textTransform: 'uppercase' }}>connected</span> : <button type="button" onClick={() => void connectGoogle()} disabled={!googleWorkspace?.configured || busy !== null} style={{ border: 0, borderRadius: 8, padding: '8px 10px', background: googleWorkspace?.configured ? 'var(--kajal)' : 'rgba(var(--rgb-ink),0.1)', color: googleWorkspace?.configured ? 'var(--paper)' : 'rgba(var(--rgb-ink),0.4)', fontSize: 9.5, fontWeight: 740, cursor: googleWorkspace?.configured ? 'pointer' : 'default' }}>{busy === 'google' ? 'Waiting...' : 'Connect'}</button>}
                    </div>
                    {!googleWorkspace?.configured && googleWorkspace?.can_configure && <div style={{ display: 'grid', gap: 7, marginTop: 9 }}>
                      <div style={{ fontSize: 9.5, lineHeight: 1.4, color: '#9a5b12' }}>Owner setup: create one Google OAuth web client with redirect <code>{`${window.location.origin}/google/callback`}</code> (open this page on your public Narad address first), then save it here. The secret stays in the host keychain.</div>
                      <input value={googleClientId} onChange={event => setGoogleClientId(event.target.value)} placeholder="Google OAuth client ID" aria-label="Google OAuth client ID" style={{ width: '100%', padding: '8px 9px', borderRadius: 8, border: '1px solid rgba(var(--rgb-ink),0.13)', background: 'var(--paper)', fontSize: 10.5 }} />
                      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 7 }}>
                        <input type="password" value={googleClientSecret} onChange={event => setGoogleClientSecret(event.target.value)} placeholder="Google OAuth client secret" aria-label="Google OAuth client secret" style={{ minWidth: 0, padding: '8px 9px', borderRadius: 8, border: '1px solid rgba(var(--rgb-ink),0.13)', background: 'var(--paper)', fontSize: 10.5 }} />
                  ...
                  <div style={{ padding: '13px 14px', borderRadius: 13, border: `1px solid ${desktopGranted ? 'rgba(53,94,59,0.25)' : 'rgba(var(--rgb-ink),0.12)'}`, background: 'rgba(var(--rgb-surface),0.5)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '36px minmax(0,1fr) auto', gap: 11, alignItems: 'center' }}>
                      ...
                      <span><span style={{ display: 'block', fontSize: 12, fontWeight: 760, color: 'var(--kajal)' }}>This Mac via CUA</span><span style={{ display: 'block', marginTop: 2, fontSize: 9.5, lineHeight: 1.35, color: 'rgba(var(--rgb-ink),0.5)' }}>{desktopRuntime?.available ? desktopRuntime.reason || 'Cua Driver is ready for confirmed desktop tasks.' : desktopRuntime?.reason || 'Cua Driver is not ready.'}</span></span>
                      {desktopGranted ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'var(--tulsi)', textTransform: 'uppercase' }}>allowed</span> : !isOwner ? <span style={{ fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.5)' }}>Ask the owner</span> : <button type="button" onClick={() => void grantInteractionTarget('cua', 'host-primary', 'Narad host desktop')} disabled={!desktopRuntime?.available || busy !== null} style={{ border: 0, borderRadius: 8, padding: '8px 10px', background: desktopRuntime?.available ? 'var(--nila)' : 'rgba(var(--rgb-ink),0.1)', color: desktopRuntime?.available ? '#fff' : 'rgba(var(--rgb-ink),0.4)', fontSize: 9.5, fontWeight: 740 }}>Allow</button>}
                    </div>
                  </div>

                  <div style={{ padding: '13px 14px', borderRadius: 13, border: '1px solid rgba(var(--rgb-ink),0.12)', background: 'rgba(var(--rgb-surface),0.5)' }}>
                    ...
                    {androidDevices.map((device, index) => {
                      ...
                      if (!isOwner) return <div key={`${externalId}:${index}`} style={{ marginTop: 8, padding: '8px 10px', borderRadius: 9, background: granted ? 'rgba(53,94,59,0.06)' : 'transparent', color: granted ? 'var(--tulsi)' : 'rgba(var(--rgb-ink),0.55)', fontSize: 10.5 }}>{granted ? `${label} allowed for this profile` : `Ask the Narad owner to allow ${label} for you`}</div>
                      return <button key={`${externalId}:${index}`} type="button" onClick={() => void grantInteractionTarget('artemis', externalId, label)} disabled={granted || busy !== null} style={{ width: '100%', marginTop: 8, padding: '8px 10px', borderRadius: 9, border: '1px solid rgba(var(--rgb-ink),0.11)', background: granted ? 'rgba(53,94,59,0.06)' : 'transparent', color: granted ? 'var(--tulsi)' : 'rgba(var(--rgb-ink),0.7)', fontSize: 10.5, textAlign: 'left' }}>{granted ? `${label} allowed for this profile` : `Allow ${label} for this profile`}</button>
                    })}
                    {phoneRuntime?.ready && androidDevices.length === 0 && <div style={{ marginTop: 8, fontSize: 9.5, color: 'rgba(var(--rgb-ink),0.45)' }}>Enable Wireless debugging on the Android phone and pair it once with the Narad host. It will then appear here.</div>}
                  </div>
                </div>

                <div style={{ marginTop: 11, padding: '9px 11px', borderRadius: 10, background: status.readiness.jev?.available ? 'rgba(53,94,59,0.065)' : 'rgba(180,83,9,0.07)', fontSize: 10, lineHeight: 1.45, color: 'rgba(var(--rgb-ink),0.58)' }}><ShieldCheck size={12} style={{ verticalAlign: '-2px', marginRight: 6, color: status.readiness.jev?.available ? 'var(--tulsi)' : '#b45309' }} />Jev safety scorer: {status.readiness.jev?.available ? 'connected for computer and phone admission and verification.' : status.readiness.jev?.reason || 'not connected.'}</div>
                ...
                <div style={{ marginTop: 20, display: 'flex', gap: 9 }}>
                  ...
                  <button type="button" onClick={() => setStep(1)} style={{ border: 0, background: 'transparent', color: 'rgba(var(--rgb-ink),0.48)', fontSize: 10.5 }}>Back</button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div style={{ maxWidth: 530 }}>
                ...
                <p style={{ margin: '10px 0 0', fontSize: 12.5, lineHeight: 1.6, color: 'rgba(var(--rgb-ink),0.56)' }}>Start in open chat or choose a guided workflow with its own intake, progress, and feedback loop.</p>

                <div style={{ marginTop: 18, padding: '2px 13px', borderRadius: 12, border: '1px solid rgba(var(--rgb-ink),0.1)', background: 'rgba(var(--rgb-surface),0.48)' }}>
                ...
                {!researchReady && (
                  <div style={{ marginTop: 11 }}>
                    <button type="button" onClick={() => setShowExaForm(value => !value)} style={{ border: 0, background: 'transparent', padding: 0, display: 'flex', gap: 6, alignItems: 'center', color: 'var(--nila)', fontSize: 10.5, cursor: 'pointer' }}><Search size={12} /> Add Exa research now <span style={{ color: 'rgba(var(--rgb-ink),0.4)' }}>(optional)</span></button>
                    {showExaForm && (
                      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 8, marginTop: 8 }}>
                        <input type="password" value={exaKey} onChange={event => setExaKey(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void connectKey('exa', exaKey) }} aria-label="Exa API key" placeholder="Paste Exa API key" style={{ padding: '9px 10px', borderRadius: 8, border: '1px solid rgba(var(--rgb-ink),0.14)', background: 'var(--paper)', color: 'var(--kajal)', fontFamily: 'var(--font-mono)', fontSize: 11 }} />
                        <button type="button" onClick={() => void connectKey('exa', exaKey)} disabled={!exaKey.trim() || busy !== null} style={{ border: 0, borderRadius: 8, padding: '8px 12px', background: exaKey.trim() ? 'var(--nila)' : 'rgba(var(--rgb-ink),0.12)', color: exaKey.trim() ? '#fff' : 'rgba(var(--rgb-ink),0.4)', fontSize: 10.5, fontWeight: 700 }}>Connect</button>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2,minmax(0,1fr))', gap: 9, marginTop: 23 }}>
                  ...
                  <button type="button" onClick={() => void finish('workspaces', !brainReady)} disabled={busy !== null} style={{ display: 'grid', gridTemplateColumns: '34px minmax(0,1fr) auto', gap: 9, alignItems: 'center', padding: '11px 12px', borderRadius: 11, border: '1px solid rgba(var(--rgb-ink),0.14)', background: 'rgba(var(--rgb-surface),0.55)', color: 'var(--kajal)', textAlign: 'left', cursor: busy ? 'wait' : 'pointer' }}><Workflow size={17} style={{ color: 'var(--sindoor)' }} /><span><span style={{ display: 'block', fontSize: 11.5, fontWeight: 750 }}>Explore workflows</span><span style={{ display: 'block', marginTop: 1, fontSize: 9, color: 'rgba(var(--rgb-ink),0.48)' }}>Career, health, teach, more</span></span><ArrowRight size={13} /></button>
                </div>
                <button type="button" onClick={() => setStep(2)} style={{ marginTop: 14, border: 0, background: 'transparent', padding: 0, color: 'rgba(var(--rgb-ink),0.45)', fontSize: 10.5, cursor: 'pointer' }}>Back to Google and devices</button>
              </div>
            )}
