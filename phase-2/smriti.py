...
Embedding provider: Gemini text-embedding-005 (768 dims) by default.
Set SMRITI_EMBEDDING_MODEL=openai to fall back to text-embedding-3-small (1536 dims).
...
_TABLE = "memories"
_SMRITI_EMBED_PROVIDER = os.environ.get("SMRITI_EMBEDDING_MODEL", "gemini").lower()
_EMBED_DIM = 768 if _SMRITI_EMBED_PROVIDER != "openai" else 1536
...
def recall_exact(query_str: str, user_id: str, avatar: str | None = None, limit: int = 3) -> str:
    ...
    Best for Parashurama (code snippets) and Narasimha (error messages)
